...
{
	log_dbg("hwnd=%p rect={(%d,%d),(%d,%d)} more=%s\n",
		...
		FAR void *arg)
{
	...
	log_dbg("hwnd=%p size=(%d,%d) pos=(%d,%d) bounds={(%d,%d),(%d,%d)}\n",
...
int nr_display_clear_row(int row)
{
	...
	rect.pt2.x = 128;
	rect.pt2.y = pos.y + 8;
	nx_fill(g_nxhello.hbkgd, &rect, 0);
#endif
	return OK;
}
